	...
	for(i=0;i<3;i++)
        {
         ...
         gas->vel[j][i]=((double)rand()/RAND_MAX)*(100.0) + 500.0;
        }
...
//AHORA TRATANDO LOS CHOQUES ENTRE PARTICULA-PARTICULA !!!
double vel_ante[N][3];
for(i=0;i<N;i++){  // HACIENDO UN BUFFER CON LAS VELOCIDADES ACTUALES ANTES DE LOS CHOQUES !!!
	for(j=0;j<3;j++)
		vel_ante[i][j]=vel[i][j] ;
}
	...
	for(j=0;j<n;j++)
        {
		if( (i!=j) && (modulo_vector(resta(pos[j],pos[i]) )<2.0*RADIO) ) {
			choque_efecto_unilateral(vel[i],vel_ante[j],pos[i],pos[j]);			
		}
        }
...
//	if( ( (pos[i][0]>=(100.0-RADIO)) || (pos[i][0]<=(-100.0+RADIO)) ) && ((pos[i][1]<=(100.0-RADIO)) && (pos[i][1]>=(-100.0+RADIO)) ) && ( (pos[i][2]<=(100.0-RADIO)) && (pos[i][2]>=(-100.0+RADIO))))
	if(  (pos[i][0]>=(100.0-RADIO)) || (pos[i][0]<=(-100.0+RADIO)) )
...
//	if(( (pos[i][1]>=(100.0-RADIO)) || (pos[i][1]<=(-100.0+RADIO)) ) && ((pos[i][0]<=(100.0-RADIO)) && (pos[i][0]>=(-100.0+RADIO))) && ((pos[i][2]<=(100.0-RADIO)) && (pos[i][2]>=(-100.0+RADIO))))
	if( (pos[i][1]>=(100.0-RADIO)) || (pos[i][1]<=(-100.0+RADIO)) )		  
		vel[i][1] =(-1.0)*vel[i][1];
//	if( ((pos[i][2]>=(100.0-RADIO)) || (pos[i][2]<=(-100.0+RADIO))) &&  ((pos[i][1]<=(100.0-RADIO)) && (pos[i][1]>=(-100.0+RADIO))) && ((pos[i][0]<=(100.0-RADIO)) && (pos[i][0]>=(-100.0+RADIO))) )
	if( (pos[i][2]>=(100.0-RADIO)) || (pos[i][2]<=(-100.0+RADIO)))
		  vel[i][2] =(-1.0)*vel[i][2];
